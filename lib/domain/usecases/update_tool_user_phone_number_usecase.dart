...
class UpdateToolUserPhoneNumberUseCase
    implements UseCase<int?, UpdateToolUserPhoneNumberParams> {
  final ToolUsersRepo _toolUsersRepo;

  UpdateToolUserPhoneNumberUseCase({ToolUsersRepo? toolUsersRepo})
      : _toolUsersRepo = toolUsersRepo ?? locator<ToolUsersRepoImp>();

  @override
  Future<int?> call(UpdateToolUserPhoneNumberParams p) {
    return _toolUsersRepo.updateToolUserPhoneNUmber(
        p.toolUserPhoneNumber, p.toolUserId);
  }
}
...
class UpdateToolUserPhoneNumberParams {
  ...
  UpdateToolUserPhoneNumberParams(
      {required this.toolUserPhoneNumber, required this.toolUserId});
}
